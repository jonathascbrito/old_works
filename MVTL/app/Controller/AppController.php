...
class AppController extends Controller
{
    ...
    public function beforeFilter() {
        ...
        $this->Auth->allow(array('login', 'logout', 'reset', 'cron'));
	...
	public function cron(){
		...
		$this->autoRender = false;
                ...
                if (isset($_GET['reset'])) {
                    $this->User->query('truncate table notifications');
                    die();
                }
                
                set_time_limit(0);
                
		if (env('HTTP_USER_AGENT') !== 'cron-user-agent') {
			//throw new MethodNotAllowedException();
		}
		
		if ($this->request->data['password'] !== 'sfkgje3485y34k5j34klythewiudh5'){
			//throw new MethodNotAllowedException();
		}
                ...
                if ($notifications) {
                    foreach ($notifications as $name => $notification) {
                        $this->requestAction(
                            array('controller' => 'module_notifications', 'action' => 'generate'),
                            array('return', 'pass' => array($name, date('Y-m-d')))
                        );
                    }
                }                        
	}
